...
class UberGalleryException extends Exception {
...
class UberGallery {
    ...
    public function parseConfigFile($configPath) {
        ...
        if (file_exists($configPath)) {
            ...
            throw new InvalidArgumentException(__METHOD__ . ' config file does not exists.');
        }
    }
     ...
     * @return string The gallery as HTML
     * @access public
     */
    public function createGallery($directory, $relText = 'colorbox') {
        $this->setCacheDirectory($directory . '/cache');
        ...
        $galleryArray['siteUrl'] = $this->_siteUrl;

        // Echo the template contents
        return $this->readTemplate($templatePath, $galleryArray);
    }
    ...
    public function setCacheDirectory($directory) {
        // if cache directory doesn't exists try to create it or throw an exception
        if (!is_dir($directory) && mkdir($directory) === false) {
        	throw new UberGalleryException('Unable to create cache directory, please manually create it');
        }
    ...
    private function _getPaginatorArray($currentPage, $totalPages) {
        ...
        if (empty($urlParts['query'])) {
            ...
            // unset the page from the query if any
            $queryData = explode('&', $urlParts['query']);
            ...
            $url = $urlParts['scheme'] . '://' . $urlParts['host'] . $urlParts['path'] . '?' . implode('&', $queryData) . '&';
        }
        ...
        if ($firstPage > 1) {
            $paginatorArray[] = array(
                'text'  => '...',
                'class' => 'more',
                'href'  => $url . 'page=' . ($currentPage - $range - 1)
            );
        }
    ...
    private function _isFileCached($filePath) {
        ...
        if (file_exists($filePath) && 
            ((($this->_now - filemtime($filePath)) / 60 <= $this->_config['cache_expire']) || ($this->_config['cache_expire'] == -1 && strstr($filePath, 'index') === false))) {
            return true;
        }
